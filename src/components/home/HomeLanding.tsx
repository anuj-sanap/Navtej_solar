  ...
  return (
    <>
      ...
      <section className="bg-white py-24 sm:py-32" aria-labelledby="steps-heading">
        <Container><div className="grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#b07d00]">A clearer way to go solar</p><h2 id="steps-heading" className="mt-4 max-w-md text-4xl font-semibold leading-tight text-brand-primary sm:text-5xl">Powering your home in 4 simple steps.</h2><p className="mt-5 max-w-md leading-7 text-slate-600">Your solar journey should feel exciting, not complicated. Our local team stays with you at every stage.</p><Link href="#quote" className="mt-7 inline-flex items-center text-sm font-bold text-brand-primary hover:text-[#b07d00]">Start with a free site visit <span className="ml-2">→</span></Link></div><div className="grid gap-3 sm:grid-cols-2">{steps.map(([number, title, description], index) => <button type="button" key={number} onClick={() => setOpenStep(index)} className={`text-left rounded-2xl border p-5 transition-all ${openStep === index ? 'border-brand-primary bg-brand-primary text-white shadow-[0_14px_30px_rgba(11,37,90,.16)]' : 'border-slate-200 bg-[#f7f8fa] text-brand-primary hover:-translate-y-1 hover:border-brand-primary/30'}`}><span className={`text-xs font-bold ${openStep === index ? 'text-brand-secondary' : 'text-[#b07d00]'}`}>{number}</span><h3 className="mt-4 text-xl font-semibold">{title}</h3><p className={`mt-2 text-sm leading-6 ${openStep === index ? 'text-white/75' : 'text-slate-500'}`}>{description}</p></button>)}</div></div></Container>
      </section>
